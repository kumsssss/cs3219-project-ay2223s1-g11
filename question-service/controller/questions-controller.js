...
import { storeQuestion, getQuestion, redisClient as redis, expireQuestion } from "../redis/datastore.js";
...
export async function viewRandomQuestion(req, res) {
    try {
        ...
        if (!room) {
            res.status(404).json({ message: "RoomId must be provided when requesting a question"});
        }
        let key = room + "key";
...
export async function viewQuestionByDifficulty(req, res) {
    try {
        ...
        return res.status(500).json({ message: `Database failure when getting ${req.params.difficulty} question` });
    }
};
